...
class Normalize_Down(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel: int):
        ...
        self.norm = nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1)
...
class RCL_Block(nn.Module):
    def __init__(
        ...
        self.conv1 = conv_l(
            filters_in, filters_out, kernel_size=kernel_size, padding=1
        )
        self.stack = nn.Sequential(
            nn.BatchNorm2d(filters_out, momentum=0.9997), nn.PReLU(), dp(dp_rate)
        )
        self.conv2 = conv_l(
            filters_out, filters_out, kernel_size=kernel_size, padding=1
        )
        self.depth = depth
