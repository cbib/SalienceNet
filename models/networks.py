...
import pytorch_ssim
from GradLoss import GradientLoss
import unet_parts_edgar as up
###############################################################################
# Helper Functions
###############################################################################
...
def define_G_A(input_nc, output_nc, ngf, netGA, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    """Create a generator
    ...
    if netGA == 'resnet_9blocks':
        net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
    elif netGA == 'resnet_6blocks':
        ...
        net = Unet3plusGenerator(filters=64, classes=1)
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % netGA)
    return init_net(net, init_type, init_gain, gpu_ids)

def define_G_B(input_nc, output_nc, ngf, netGB, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    """Create a generator
    ...
    if netGB == 'resnet_9blocks':
        net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
    elif netGB == 'resnet_6blocks':
        net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6)
    elif netGB == 'unet_128':
        net = UnetGenerator(input_nc, output_nc, 7, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netGB == 'unet_256':
        ...
        raise NotImplementedError('Generator model name [%s] is not recognized' % netGB)
    return init_net(net, init_type, init_gain, gpu_ids)
...
class GANLoss(nn.Module):
    ...
    def __init__(self, gan_mode, wcrit1, wcrit2, wcrit3, target_real_label=1.0, target_fake_label=0.0):
        ...
        elif gan_mode == 'Gradient':
            self.loss == GradientLoss()
        elif gan_mode == 'LSSSIMGRAD':
            self.loss = None
        else:
    ...
    def __call__(self, prediction, target_is_real):
        ...
        Returns:
            the calculated loss.
        """
        if self.gan_mode in ['lsgan', 'vanilla', 'SSIM', 'Gradient']:
            target_tensor = self.get_target_tensor(prediction, target_is_real)
            loss = self.loss(prediction, target_tensor)
        elif self.gan_mode == "LSSSIMGRAD":

            target_tensor = self.get_target_tensor(prediction, target_is_real)

            crit1 = nn.MSELoss()
            crit2 = pytorch_ssim.SSIM()
            crit3 = GradientLoss()

            # loss = crit1(prediction, target_tensor) + crit2(prediction, target_tensor) + crit3(prediction, target_tensor)

            loss1 = self.wcrit1 * crit1(prediction, target_tensor)
            loss2 = self.wcrit2 * crit2(prediction, target_tensor)
            loss3 = self.wcrit3 * crit3(prediction, target_tensor)

            loss = loss1 + loss2 + loss3


            # loss = crit1*crit1_factor + crit2*crit2_factor + crit3*crit3_factor
...
class Unet3plusGenerator(
    ...
    def __init__(
        ...
        self.model = up.OutConv(160, classes, conv_l=conv_l)
